...
const resources = ["invoices", "reviews", "users", "settings", "audit-logs", "features", "gallery"] as const;
...
export default async function SuperadminResourcePage({ params }: { params: Promise<{ resource: string }> }) {
  ...
  const canManage =
    resource === "users" ? admin.permissions.canManageAdmins
    : resource === "settings" ? admin.permissions.canManageSettings
    : resource === "audit-logs" ? false
    ...
    : admin.permissions.canManageGallery;
  const canRead =
    resource === "users" ? admin.permissions.canManageAdmins
    : resource === "settings" ? admin.permissions.canManageSettings
    : resource === "audit-logs" ? admin.permissions.canViewReports
    ...
    : admin.permissions.canReadGallery;
