        ...
        <AnimatedCard className="p-8">
          <div className="grid gap-8 lg:grid-cols-[1fr_0.9fr]">
            <div>
              ...
              <p className="mt-4 text-lg text-[color:var(--muted)]">Choose a secure payment option and complete your booking.</p>
              <div className="mt-8 grid gap-3">
                {paymentMethods.map((method) => (
                  <button key={method} className="flex items-center justify-between rounded-3xl border border-white/10 card-surface px-4 py-4 text-left text-sm text-[color:var(--muted)] transition hover:border-[color:rgba(16,185,129,0.12)] hover:bg-[color:rgba(16,185,129,0.06)]">
                    <div>
                      <p className="font-semibold text-white">{method}</p>
                      <p className="mt-1 text-[color:var(--muted)]">Secure and fast checkout using {method}.</p>
                    </div>
