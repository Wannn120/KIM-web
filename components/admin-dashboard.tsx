...
export default function AdminDashboard({
  ...
  return (
    <main className="flex-1 px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
        ...
        <div className="rounded-[2rem] border border-white/10 bg-[color:var(--surface-strong)] p-6 sm:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-semibold text-white sm:text-3xl">{admin.role.replace("_", " ")}</h1>
          ...
          {admin.permissions.canViewReports ? (
            <>
              <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6">
                ...
                <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">Rp {summary.revenueToday.toLocaleString("id-ID")}</p>
              </div>
              <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6">
                <p className="text-sm text-[color:var(--muted)]">Revenue this month</p>
                <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">Rp {summary.revenueThisMonth.toLocaleString("id-ID")}</p>
              </div>
              <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6">
                <p className="text-sm text-[color:var(--muted)]">Bookings today</p>
                <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">{summary.bookingsToday}</p>
              </div>
              <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6">
                <p className="text-sm text-[color:var(--muted)]">Bookings this month</p>
                <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">{summary.bookingsThisMonth}</p>
              </div>
            </>
          ) : (
            <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6 sm:col-span-2 xl:col-span-3">
        ...
        {!admin.permissions.canViewReports ? (
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6">
              ...
              <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">{summary.pendingBookings}</p>
            </div>
            {admin.permissions.canReadPayments ? (
              <div className="rounded-[1.5rem] border border-white/10 bg-[color:var(--surface)] p-5 sm:p-6">
                <p className="text-sm text-[color:var(--muted)]">Pending payments</p>
                <p className="mt-3 text-2xl font-semibold text-white sm:text-3xl">{summary.pendingPayments}</p>
              </div>
            ) : null}
          </div>
        ) : null}
